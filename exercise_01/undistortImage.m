...
num_pixels = height * width; % Total pixel count
...
pixels_distorted_integer = round(pixels_distorted); % Round pixel coordinates to nearest integer values for nearest-neighbor interpolation
pixel_grayscales = img(height*pixels_distorted_integer(1, :) + pixels_distorted_integer(2, :)); % Read out pixel greyscale values at NN-coordinates
img_undistorted = uint8(reshape(pixel_grayscales, [height, width])); % Reshape NN pixel values into image shape, convert to 256-bit greyscale
end