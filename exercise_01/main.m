...
% Camera poses
poses = load('./data/poses.txt'); % N camera pose vectors [N x 6], see problem statement
...
P_W_corners = [P_W_corners'; % Transpose to column vectors,
               zeros(1, num_corners_x*num_corners_y)];  % Add z coordinate of zeroes
...
% Show result on distorted image
figure()
imshow(img);
hold on;
plot(projected_corners(1, :), projected_corners(2, :), 'r .', 'MarkerSize', 10);
hold off;
title('Distorted image with projected corners');



% Undistort image and show results
img_undistorted = undistortImage(img, K, D); 
figure()
imshow(img_undistorted);
title('Undistorted image');


% Generate and draw cube on undistorted image
cube_points = generateCube(3*square_size, 1*square_size, 2*square_size, C_T_W, K);
drawCube(cube_points, img_undistorted);
